...
// Strategy: 2-4 blocks/SM × 64-96 threads = 4-12 warps SM (conservative for constrained systems)
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== GPUtronic Stage 13 — Blackwell v4 FIXED ===\\n";
    ...
    std::cout << "Warp Scheduling: " << BLOCKS_PER_SM << " blocks/SM × " 
              << BLOCK_THREADS << " threads = " << (BLOCKS_PER_SM * BLOCK_THREADS/32) 
              << " warps SM (~" << (100.0 * BLOCKS_PER_SM * BLOCK_THREADS/32 / 48) << "% of max 48)\\n\\n";
    ...
    // Set device flags for better pinned memory allocation
    ...
    float* h_q = nullptr;
    uint64_t* h_global_work = nullptr;
    uint64_t* h_local_work = nullptr;

    std::cout << "[SETUP] Allocating zero-copy memory (throttle cable)...\\n";

    // Allocate pinned host memory and get device pointers
    ...
    float* d_q = nullptr;
    err = cudaHostGetDevicePointer((void**)&d_q, (void*)h_q, 0);
    if (err != cudaSuccess) {
        std::cerr << "[ERROR] cudaHostGetDevicePointer q failed: " << cudaGetErrorString(err) << "\\n";
        cudaFreeHost(h_q); h_q = nullptr;
        return 1;
    }
    
    err = cudaMallocHost((void**)&h_global_work, sizeof(uint64_t));
    if (err != cudaSuccess) {
        std::cerr << "[ERROR] cudaMallocHost global failed: " << cudaGetErrorString(err) << "\\n";
        cudaFreeHost(h_q); h_q = nullptr;
        return 1;
    }
    
    uint64_t* d_global_work_ptr = nullptr;
    err = cudaHostGetDevicePointer((void**)&d_global_work_ptr, (void*)h_global_work, 0);
    if (err != cudaSuccess) {
        std::cerr << "[ERROR] cudaHostGetDevicePointer global failed: " << cudaGetErrorString(err) << "\\n";
        cudaFreeHost(h_q); h_q = nullptr;
        cudaFreeHost(h_global_work); h_global_work = nullptr;
        return 1;
    }

    err = cudaMallocHost((void**)&h_local_work, sm_count * sizeof(uint64_t));
    if (err != cudaSuccess) {
        std::cerr << "[ERROR] cudaMallocHost local failed: " << cudaGetErrorString(err) << "\\n";
        cudaFreeHost(h_q); h_q = nullptr;
        cudaFreeHost(h_global_work); h_global_work = nullptr;
        return 1;
    }
    
    uint64_t* d_local_work_ptr = nullptr;
    err = cudaHostGetDevicePointer((void**)&d_local_work_ptr, (void*)h_local_work, 0);
    if (err != cudaSuccess) {
        std::cerr << "[ERROR] cudaHostGetDevicePointer local failed: " << cudaGetErrorString(err) << "\\n";
        cudaFreeHost(h_q); h_q = nullptr;
        cudaFreeHost(h_global_work); h_global_work = nullptr;
        cudaFreeHost(h_local_work); h_local_work = nullptr;
        return 1;
    }

    // Kernel launch setup - device-side pointers
    uint64_t* d_global_work_dev;
    uint64_t* d_local_work_dev;
    
    err = cudaMalloc(&d_global_work_dev, sizeof(uint64_t));
    if (err != cudaSuccess) {
    ...
    err = cudaMalloc(&d_local_work_dev, sm_count * sizeof(uint64_t));
    if (err != cudaSuccess) {
        std::cerr << "[ERROR] cudaMalloc local work failed: " << cudaGetErrorString(err) << "\\n";
        cudaFreeHost(h_q); h_q = nullptr;
        cudaFreeHost(h_global_work); h_global_work = nullptr;
        cudaFreeHost(h_local_work); h_local_work = nullptr;
        cudaFree(d_global_work_dev);
        return 1;
    }
    
    cudaMemset(d_global_work_dev, 0, sizeof(uint64_t));
    ...
    std::cout << "[LAUNCH] SCAG engine: " << sm_count << " SMs × " << BLOCKS_PER_SM 
              ...
              << " warps/SM, ~" << (100.0 * BLOCKS_PER_SM * 2 / 48) << "% of max 48 warps)\\n";
    g_scag_engine_v4<<<total_blocks, BLOCK_THREADS>>>(d_q, d_global_work_dev, d_local_work_dev, (int)mode);
