// =============================================================================
// GPUTronic Q-Axis Governor v24 — Nanosleep Throttle (RTX 5080 / sm_120)
// =============================================================================
...
__device__ __forceinline__ void bursty_workload(float* __restrict__ acc,
                                                int sm_id, int iter, float intensity) {
...
__global__ void gpu_persistent_governor_kernel(unsigned long long* sm_counters,
    GPUControlData* control_data, BurstyParams* burst_params, int num_sm) {
    ...
    while (true) {
        ...
        control_data->active_blocks_current = BLOCKS_PER_SM;
    }
}
...
void* control_loop(void* arg) {
    ...
    printf("[CTRL] v24 control loop started\n");
    ...
    while (g_running) {
        ...
        if (z > 2.0f) z = 2.0f; if (z < 0.05f) z = 0.05f;
        ...
        if (new_sleep > g_h_control_data->max_sleep_ns) new_sleep = g_h_control_data->max_sleep_ns;
        g_h_control_data->throttle_sleep_ns = new_sleep;
        ...
        if (new_sleep > BASE_SLEEP_NS) g_h_control_data->proactive_corrections++;
...
void* burst_scheduler(void* arg) {
    (void)arg;
    while (g_running) {
        g_h_burst_params->burst_active = 1; g_h_burst_params->intensity_factor = 1.45f; usleep(800000);
        g_h_burst_params->burst_active = 0; g_h_burst_params->intensity_factor = 0.55f; usleep(1200000);
    }
    return NULL;
}
...
int main() {
    printf("═══════════════════════════════════════════════════════════\n");
    printf("[GPUTronic] v24 Nanosleep Throttle Governor (RTX 5080)\n");
    printf("═══════════════════════════════════════════════════════════\n\n");
    ...
    g_h_control_data->max_sleep_ns = MAX_SLEEP_NS;
    g_h_control_data->throttle_sleep_ns = 0;
    ...
    gpu_persistent_governor_kernel<<<NUM_SM, block>>>(
        g_sm_counters, g_d_control_data, g_d_burst_params, NUM_SM);
    ...
    for (int i = 0; i < 15; i++) {
        usleep(1000000);
        printf("[TEL] t=%2ds | Z=%.3f | sleep=%6d | PM=%.3f | err=%.3f | corr=%d\n",
               i+1, g_h_control_data->z_estimate, g_h_control_data->throttle_sleep_ns,
               g_h_control_data->current_pm, g_h_control_data->pm_error,
    ...
    printf("\n[GPUTronic] v24 validation complete.\n");
