// =============================================================================
// GPUTronic Q-Axis Governor v26.3 — Hybrid (Online Max-Rate + Model)
// =============================================================================
// - 20s warm-up discovers observed_max_rate
// - Simple expected_rate = max_rate * f(sleep)
// - Z = expected_rate / current_rate (hybrid reference)
...
#define WARMUP_TIME_US      20000000.0   // 20 seconds
...
static float observed_max_rate = 0.0f;
...
void* control_loop(void* arg) {
    ...
    printf("[CTRL] v26.3 HYBRID | online max-rate + model\n");
    ...
    while (g_running) {
        ...
        // === Hybrid Z Calculation ===
        float expected_rate = observed_max_rate;
        float current_sleep = g_h_control->throttle_sleep_ns;

        if (calibration_done && observed_max_rate > 1000.0f) {
            float sleep_factor = 1.0f - (current_sleep / (float)MAX_SLEEP_NS * 0.92f);
            if (sleep_factor < 0.05f) sleep_factor = 0.05f;
            expected_rate = observed_max_rate * sleep_factor;
        }

        float z;
        if (now < warmup_until) {
            z = 1.0f;   // neutral during calibration
            if (rate_ema > observed_max_rate) observed_max_rate = rate_ema;
        } else {
            calibration_done = 1;
            z = expected_rate / (rate_ema + 1.0f);
        }
    ...
    if (csv) fclose(csv);
    printf("[INFO] observed_max_rate = %.1f\n", observed_max_rate);
    return NULL;
}
...
int main(int argc, char** argv) {
    ...
    printf("═══════════════════════════════════════════════════════════\n");
    printf("[GPUTronic v26.3 HYBRID] Online max-rate + sleep model\n");
    printf("═══════════════════════════════════════════════════════════\n\n");
    ...
    for (int i = 0; i < 300; i++) {
        usleep(1000000);
        if (!g_running) break;
        printf("[TEL] t=%3ds | Z=%.3f | sleep=%6d | max_rate=%.0f\n",
               i+1, g_h_control->z_estimate, g_h_control->throttle_sleep_ns, observed_max_rate);
    }
    ...
    printf("\n[GPUTronic v26.3 HYBRID] Run complete. CSV: gputronic_log.csv\n");
